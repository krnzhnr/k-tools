...
def test_muxer_grouping_and_execution(mocker, temp_dir):
    ...
    # Execute
    settings = {"subs_title": "Rus", "clean_tracks": True}
    ...
    mocker.patch(
        "app.core.settings_manager.SettingsManager.use_auto_subfolder",
    ...
    # Check audio args
    audio_input = next(i for i in inputs if i["path"] == audio)
    assert "--default-track" in audio_input["args"]
    assert "--forced-display-flag" in audio_input["args"]

    # Check subs args
    subs_input = next(i for i in inputs if i["path"] == subs)
    assert "--track-name" in subs_input["args"]
    assert "0:Rus" in subs_input["args"]
    assert "--default-track" in subs_input["args"]
    assert "--forced-display-flag" in subs_input["args"]
