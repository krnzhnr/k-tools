...
def test_ffmpeg_termination_on_error(mocker):
    ...
    mock_popen.communicate.return_value = (b"", "Критическая ошибка FFmpeg".encode("utf-8"))
...
def test_infrastructure_path_isolation(mocker):
    ...
    mocker.patch("app.core.path_utils.get_binary_path", return_value="C:\\tools\\ffmpeg.exe")
...
def test_ffmpeg_generic_exception(mocker):
    ...
    mocker.patch("app.core.path_utils.get_binary_path", return_value="ffmpeg")
    mocker.patch("subprocess.run", side_effect=RuntimeError("Generic crash"))
