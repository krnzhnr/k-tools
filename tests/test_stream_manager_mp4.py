...
def mock_tracks():
    return [
        TrackInfo(track_id=0, track_type="video", codec="h264", language="und", name="Video"),
        TrackInfo(track_id=1, track_type="audio", codec="aac", language="und", name="Audio AAC"),
        TrackInfo(track_id=2, track_type="audio", codec="ac3", language="und", name="Audio AC3"),
    ]
...
def test_meta_data_updated(script):
    assert "MP4" in script.name
    assert ".mp4" in script.file_extensions
    assert "MP4" in script.description
