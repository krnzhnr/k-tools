...
def test_script_page_combo_checkbox(qtbot):
    class ComplexMockScript(MockScript):
        ...
        def settings_schema(self):
            return [
                SettingField("combo", "Combo", SettingType.COMBO, default="B", options=["A", "B", "C"]),
                SettingField("check", "Check", SettingType.CHECKBOX, default=True)
            ]
...
def test_script_page_dynamic_visibility(qtbot):
    class VisibilityMockScript(MockScript):
        ...
        def settings_schema(self):
            return [
                SettingField("trigger", "Trigger", SettingType.CHECKBOX, default=False),
                SettingField("target", "Target", SettingType.TEXT, visible_if={"trigger": [True]})
            ]
