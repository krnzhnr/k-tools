...
from app.scripts.stream_manager import StreamManagerScript, MODE_KEEP, MODE_REMOVE
...
def mock_tracks():
    return [
        TrackInfo(track_id=0, track_type="video", codec="HEVC", language="und", name="Video"),
        TrackInfo(track_id=1, track_type="audio", codec="AC-3", language="und", name="Audio AC3"),
        TrackInfo(track_id=2, track_type="audio", codec="DTS", language="und", name="Audio DTS"),
    ]
...
def test_extract_single_audio_raw_ac3(script, mock_tracks, mocker, tmp_path):
    ...
    mock_mkvmerge = mocker.patch.object(script._runner, "run", return_value=True)
...
def test_extension_mapping(script, mocker, tmp_path):
    ...
    for codec, expected_ext in codecs:
        track = TrackInfo(track_id=1, track_type="audio", codec=codec, language="und", name="Test")
