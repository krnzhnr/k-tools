...
class TestCodecTypeMap:
    ...
    def test_subtitle_mapped(self) -> None:
        ...
        assert _CODEC_TYPE_MAP["subtitle"] == "subtitles"
...
class TestFFProbeRunnerInit:
    ...
    def test_init_sets_path(self, mocker) -> None:
        ...
        assert runner._ffprobe_path == "C:\\bin\\ffprobe.exe"
...
class TestFFProbeRunnerGetStreams:
    ...
    def test_success_two_streams(self, mocker) -> None:
        ...
        mock_popen.communicate.return_value = (_ffprobe_json(raw), "")
        mock_popen._was_cancelled = False
        mocker.patch("subprocess.Popen", return_value=mock_popen)

        streams = runner.get_streams(Path("test.mp4"))
    ...
    def test_subtitle_type_remapped(self, mocker) -> None:
        ...
        streams = runner.get_streams(Path("test.mkv"))
    ...
    def test_empty_streams(self, mocker) -> None:
        ...
        mock_popen = MagicMock()
        mock_popen.returncode = 0
        mock_popen.communicate.return_value = (_ffprobe_json([]), "")
        mock_popen._was_cancelled = False
        mocker.patch("subprocess.Popen", return_value=mock_popen)

        streams = runner.get_streams(Path("empty.mp4"))
        assert streams == []
    ...
    def test_missing_tags(self, mocker) -> None:
        ...
        mock_popen.communicate.return_value = (
            json.dumps({"streams": [raw_stream]}),
            "",
        )
        mock_popen._was_cancelled = False
        mocker.patch("subprocess.Popen", return_value=mock_popen)

        streams = runner.get_streams(Path("no_tags.mp4"))
        assert streams[0].language == "und"
        assert streams[0].name == ""

    def test_nonzero_returncode_raises(self, mocker) -> None:
        ...
        mock_popen.communicate.return_value = ("", "Invalid data")
        mock_popen._was_cancelled = False
        mocker.patch("subprocess.Popen", return_value=mock_popen)

        with pytest.raises(RuntimeError):
            runner.get_streams(Path("broken.mp4"))

    def test_invalid_json_raises(self, mocker) -> None:
    ...
    def test_file_not_found_raises(self, mocker) -> None:
        ...
        with pytest.raises(FileNotFoundError):
            runner.get_streams(Path("test.mp4"))

    def test_unexpected_exception_raises(self, mocker) -> None:
    ...
    def test_unknown_codec_type_passthrough(self, mocker) -> None:
        ...
        streams = runner.get_streams(Path("data.mp4"))
        assert streams[0].stream_type == "data"

    def test_three_streams_order(self, mocker) -> None:
        ...
        raw = [
            _make_raw_stream(index=0, codec_type="video"),
            _make_raw_stream(index=1, codec_type="audio"),
            _make_raw_stream(
                index=2,
                codec_type="subtitle",
            ),
        ...
        streams = runner.get_streams(Path("full.mp4"))
        assert len(streams) == 3
        indices = [s.stream_index for s in streams]
        assert indices == [0, 1, 2]
        types = [s.stream_type for s in streams]
        assert types == ["video", "audio", "subtitles"]
