...
### Шаг 6.1 — Создание IDialogService и привязка к XamlRoot
...
| 2 | Изоляция зависимостей (DI) | 6 | ⏳ В РАБОТЕ | Устранение Service Locator и синглтонов `.Instance`. |
| 3 | Многопоточность и DispatcherQueue | 2 | 📅 ПЛАН | 0 блокировок UI, защита от `COMException`. |
| 4 | Рефакторинг представления (x:Bind) | 2 | 📅 ПЛАН | Переход на скомпилированные привязки и разгрузка Code-Behind. |
| 5 | Работа с сетью (IHttpClientFactory) | 2 | 📅 ПЛАН | Устранение проблем сокетов, интеграция Polly. |
| 6 | Модальные окна (ContentDialog) | 1 | 📅 ПЛАН | Абстракция IDialogService с привязкой XamlRoot. |
